...
# Helper function to count fails in columns
def count_fails(ng_df, features):
    """
    Count the number of 'fail' values in each feature column of the NG dataframe.
    
    Args:
        ng_df (pd.DataFrame): DataFrame containing NG rows.
        features (list): List of feature columns to analyze.
    
    Returns:
        list: List of (column, count) tuples sorted by count descending, top 5.
    """
    fail_counts = {}
    for col in features:
        if col in ng_df.columns:
            count = (ng_df[col].astype(str).str.lower() == "fail").sum()
            fail_counts[col] = count
    top_5 = sorted(fail_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    return top_5
        ...
        try:
            ...
            for station, model in self.selected:
                if self.isInterruptionRequested():
                    ...
                    return
                if station not in AppState.features:
                    self.log_signal.emit(f"Warning: Station {station} not found in features")
                    continue
                try:
                    features = AppState.features[station]["models"][model]["features"]
                except KeyError:
                    self.log_signal.emit(f"Warning: Model {model} not found for station {station}")
                    continue
                df = AppState.retrieved_dfs.get(station, pd.DataFrame()).copy()
                if df.empty:
                    self.log_signal.emit(f"No data for station {station}")
                    continue
                self.log_signal.emit(f"Analyzing station: {station}, model: {model}, rows: {len(df)}")
                ng = df[df["Result"].str.upper() == "NG"]
                if ng.empty:
                    analyzed_dfs[station] = []
                    continue
                top_5 = count_fails(ng, features)
                analyzed_dfs[station] = top_5
                current_progress += len(df)
                self.progress.emit(int(current_progress * 100 / total_rows) if total_rows > 0 else 0)
    ...
    def run(self):
        try:
            ...
            if not all([host, user, db]):
                self.log_signal.emit("Error: Incomplete database configuration (host, user, or database missing)")
                self.error.emit("Incomplete database configuration")
                return
            ...
            try:
                ...
                self.log_signal.emit(f"Connected to database {db} on {host}:{port}")
            except Exception as e:
                self.log_signal.emit(f"Error: Database connection failed: {e}")
                self.error.emit(f"Database connection failed: {e}")
                return

            # Load features.json
            try:
                if os.path.exists("JSON_Files/features.json"):
                    with open("JSON_Files/features.json", "r") as f:
                        AppState.features = json.load(f)
                    self.log_signal.emit(f"Loaded features.json with stations: {list(AppState.features.keys())}")
                else:
                    self.log_signal.emit("Error: features.json not found in JSON_Files directory")
                    AppState.features = {}
                    self.error.emit("features.json not found")
                    return
            except Exception as e:
                self.log_signal.emit(f"Error: Failed to load features.json: {e}")
                AppState.features = {}
                self.error.emit(f"Failed to load features.json: {e}")
                return

            # Load troubleshootings.json
            try:
                if os.path.exists("JSON_Files/troubleshootings.json"):
                    with open("JSON_Files/troubleshootings.json", "r") as f:
                        AppState.troubleshooting = json.load(f)
                    self.log_signal.emit(f"Loaded troubleshootings.json with stations: {list(AppState.troubleshooting.keys())}")
                else:
                    self.log_signal.emit("Warning: troubleshootings.json not found in JSON_Files directory")
                    AppState.troubleshooting = {}
            except Exception as e:
                self.log_signal.emit(f"Error: Failed to load troubleshootings.json: {e}")
                AppState.troubleshooting = {}
            ...
            try:
                ...
                self.log_signal.emit(f"Available database tables: {tables}")
                selected_tables = self.config.get("selected_tables", tables)
                if not selected_tables:
                    self.log_signal.emit("Error: No tables selected for auto-run")
                    self.error.emit("No tables selected for auto-run")
                    return
                AppState.selected_tables = selected_tables
                self.log_signal.emit(f"Selected tables: {selected_tables}")
            except Exception as e:
                self.log_signal.emit(f"Error: Failed to refresh tables: {e}")
                self.error.emit(f"Failed to refresh tables: {e}")
                return

            # Validate stations against features
            valid_stations = [station for station in selected_tables if station in AppState.features]
            if not valid_stations:
                self.log_signal.emit("Error: No selected tables have corresponding entries in features.json")
                self.error.emit("No valid stations found in features.json")
                return
            self.log_signal.emit(f"Valid stations with features: {valid_stations}")
            ...
            if date_str:
                try:
                    ...
                    self.log_signal.emit(f"Error: Invalid date format in config: {date_str}")
                    self.error.emit(f"Invalid date format: {e}")
                    return
            ...
            self.log_signal.emit(f"Retrieving data: state={state if state else 'None'}, from={dt_from}, to={dt_to}")

            # Retrieve data
            dfs = {}
            for table in valid_stations:
                if self.isInterruptionRequested():
                ...
                try:
                    ...
                    self.log_signal.emit(f"Retrieved {len(df)} rows from {table}")
                except Exception as e:
                    self.log_signal.emit(f"Error: Retrieve failed for {table}: {e}")
                    df = pd.DataFrame()
                df = strip_dataframe(df)
                dfs[table] = df
            AppState.retrieved_dfs = dfs
            self.log_signal.emit("Data retrieval completed")

            # Select stations and models
            selected = []
            for station in valid_stations:
                models = list(AppState.features[station].get("models", {}).keys())
                self.log_signal.emit(f"Station {station} models: {models}")
                if models:
                    model = models[0]
                    selected.append((station, model))
                else:
                    self.log_signal.emit(f"Warning: No models defined for station {station}")
            AppState.selected = selected

            if not selected:
                self.log_signal.emit("Error: No stations with models available for auto-report generation")
                self.error.emit("No stations with models available")
                return
            self.log_signal.emit(f"Selected stations and models: {selected}")
            ...
            for station, model in selected:
                try:
                    ...
                    self.log_signal.emit(f"Error: Features not found for {station}/{model}")
                    continue
                df = AppState.retrieved_dfs.get(station, pd.DataFrame()).copy()
                if df.empty:
                    self.log_signal.emit(f"No data for {station}")
                    continue
                ng = df[df["Result"].str.upper() == "NG"]
                if ng.empty:
                    analyzed_dfs[station] = []
            ...
            self.log_signal.emit("Starting report generation...")
            ...
            if date_str:
                ...
                try:
                    ...
                    self.log_signal.emit(f"Warning: Failed to update app_config.json: {e}")
            ...
            include_week = self.config.get("include_week_no", True)
            custom_filename = self.config.get("html_filename", "")
            custom_title = self.config.get("html_title", " : NG Analysis Report")

            if custom_filename:
                default_filename = f"{custom_filename}_Week_{week_no}.html" if include_week else f"{custom_filename}.html"
            else:
                default_filename = f"_NG RootCauseAnalysis_Week_{week_no}.html" if include_week else "_NG RootCauseAnalysis.html"

            if not custom_title:
                custom_title = " : NG Analysis Report"
            if include_week:
                custom_title += f" - Week {week_no}"

            auto_save_path = self.config.get("auto_save_path", "")
            path = os.path.join(auto_save_path, default_filename) if auto_save_path else default_filename
            ...
            if os.path.exists(logo_path):
                ...
                logo_src = 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/2b/Valeo_Logo.svg/2560px-Valeo_Logo.svg.png'
            ...
            html_content = [
                ...
                '<head>',
                '<style>',
                'body { font-family: Helvetica, sans-serif; margin: 0; background-color: #f3f4f6; color: #111827; zoom: 112.5%; }',
                ...
                f'<dt>Analyzed Stations:</dt><dd>{", ".join([s for s, m in selected])}</dd>',
            ...
            self.app._plot_ok_ng_combined(ax, start_str_no_hour, end_str_no_hour)
            buf = BytesIO()
            ...
            fig = Figure(figsize=(5, 3))
            ax = fig.add_subplot(111)
            self.app._plot_ng_percentage(ax, start_str_no_hour, end_str_no_hour)
            buf = BytesIO()
            fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
            buf.seek(0)
            img_data = base64.b64encode(buf.read()).decode('utf-8')
            ...
            fig = Figure(figsize=(5, 3))
            ax = fig.add_subplot(111)
            self.app._plot_ok_ng_pie(ax, start_str_no_hour, end_str_no_hour)
            buf = BytesIO()
            fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
            buf.seek(0)
            img_data = base64.b64encode(buf.read()).decode('utf-8')
            ...
            fig = Figure(figsize=(5, 3))
            ax = fig.add_subplot(111)
            self.app._plot_ng_rate_by_time(ax, start_str_no_hour, end_str_no_hour)
            buf = BytesIO()
            fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
            buf.seek(0)
            img_data = base64.b64encode(buf.read()).decode('utf-8')
            ...
            # NG Count by Model Plot
            fig = Figure(figsize=(5, 3))
            ax = fig.add_subplot(111)
            self.app._plot_ng_by_model(ax, start_str_no_hour, end_str_no_hour)
            buf = BytesIO()
            fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
            buf.seek(0)
            img_data = base64.b64encode(buf.read()).decode('utf-8')
            html_content.append('<div class="grid-item">')
            html_content.append(f'<img src="data:image/png;base64,{img_data}" alt="NG Count by Model">')
            html_content.append(f'<div class="caption">NG Count by Model</div>')
            html_content.append('</div>')
            plt.close(fig)
            ...
            for station, model in selected:
                # Skip if no features defined for this station/model
                if not AppState.features.get(station, {}).get("models", {}).get(model, {}).get("features", []):
                    continue
                fig = Figure(figsize=(3.5, 2.2))
                ax = fig.add_subplot(111)
                self.app._plot_root_cause_for_station(ax, station, model)
                buf = BytesIO()
                fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
                buf.seek(0)
                img_data = base64.b64encode(buf.read()).decode('utf-8')
            ...
            for station, model in selected:
                ...
                try:
                    ...
                    continue
                for col in features:
                    if col in ng.columns:
                        count = (ng[col].astype(str).str.lower() == "fail").sum()
                        if count > 0:
                            methods_list = AppState.troubleshooting.get(station, {}).get(col, [])
                            percentage = (count / total_ng * 100) if total_ng > 0 else 0
                            data.append([station, col, methods_list, count, percentage])
            ...
            try:
                self.log_signal.emit(f"Generating HTML report: {path}")
                with open(path, "w", encoding="utf-8") as f:
                    f.write("\n".join(html_content))
                self.log_signal.emit(f"HTML report saved to {path}")
                try:
                    webbrowser.open('file://' + os.path.realpath(path))
                    self.log_signal.emit("Opened report in browser")
                except Exception as e:
                    self.log_signal.emit(f"Warning: Failed to open browser: {e}")
            except Exception as e:
                self.log_signal.emit(f"Error: Failed to save HTML report: {e}")
                self.error.emit(f"Failed to save HTML report: {e}")
                return

            self.finished.emit()
        except Exception as e:
            self.log_signal.emit(f"Critical error in auto-run: {e}")
            self.error.emit(str(e))

class RuleAnalyzerApp(QTabWidget):
    def __init__(self):
        super().__init__()
        ...
        # Initialize tabs
        self.config_tab = ConfigTab(app=self, parent=self)
        self.addTab(self.config_tab, "Database Configuration")
    ...
    def load_app_config(self):
        if os.path.exists("JSON_Files/app_config.json"):
            try:
                with open("JSON_Files/app_config.json", "r") as f:
                    config = json.load(f)
                self.config_tab.host.setText(config.get("host", "localhost"))
                self.config_tab.port.setText(config.get("port", "3306"))
                self.config_tab.user.setText(config.get("user", "root"))
                self.config_tab.password.setText(config.get("password", ""))
                ...
                self.app_config_tab.html_title.setText(config.get("html_title", " : NG Analysis Report"))
                ...
                self.app_config_tab.tables_label.setText(
                    f"{len(self.app_config_tab.selected_tables)} tables selected" if self.app_config_tab.selected_tables else "No tables selected"
                )
                self.auto_save_path.setText(config.get("auto_save_path", ""))
                self.auto_save_chk.setChecked(True)
                log("Loaded app_config.json successfully")
            except Exception as e:
                log(f"Failed to load app_config.json: {e}", "ERROR")

        # Load features.json with validation
        if os.path.exists("JSON_Files/features.json"):
            try:
                with open("JSON_Files/features.json", "r") as f:
                    AppState.features = json.load(f)
                log(f"Loaded features.json with stations: {list(AppState.features.keys())}")
            except Exception as e:
                log(f"Failed to load features.json: {e}", "ERROR")
                AppState.features = {}
        else:
            log("features.json not found, initializing empty features", "WARN")
            AppState.features = {}
            default_features = {
                "Station1": {"models": {"ModelA": {"features": ["Feature1"]}}},
                "Station2": {"models": {"ModelX": {"features": ["Feature2"]}}}
            }
            try:
                os.makedirs("JSON_Files", exist_ok=True)
                with open("JSON_Files/features.json", "w") as f:
                    json.dump(default_features, f, indent=4)
                AppState.features = default_features
                log("Created default features.json")
            except Exception as e:
                log(f"Failed to create default features.json: {e}", "ERROR")
        ...
        if os.path.exists("JSON_Files/troubleshootings.json"):
            try:
                with open("JSON_Files/troubleshootings.json", "r") as f:
                    AppState.troubleshooting = json.load(f)
                log(f"Loaded troubleshootings.json with stations: {list(AppState.troubleshooting.keys())}")
            except Exception as e:
                log(f"Failed to load troubleshootings.json: {e}", "ERROR")
                AppState.troubleshooting = {}
        else:
            log("troubleshootings.json not found, initializing empty troubleshooting", "WARN")
            AppState.troubleshooting = {}
    ...
    def perform_auto_run(self, config):
        ...
        self.auto_worker = AutoRunWorker(config, self)
        ...
        self.auto_worker.error.connect(
            lambda err: (
                log(f"Auto-run failed: {err}", "ERROR"),
                QMessageBox.critical(self, "Auto-Run Error", f"Auto-run failed: {err}\nCheck the log for details."),
                QTimer.singleShot(5000, lambda: (self.log_dlg.close(), QApplication.quit()))
            )
        )
    ...
    def _plot_ok_ng_combined(self, ax, start_str_no_hour, end_str_no_hour):
        ...
        max_height = max(np.array(ok_counts) + np.array(ng_counts)) if len(ok_counts) > 0 else 0
    ...
    def _plot_ng_rate_by_time(self, ax, start_str_no_hour, end_str_no_hour):
        ...
        ax.set_title(f"NG Rate by Time", fontsize=10, fontweight='medium')
    ...
    def _plot_ng_by_model(self, ax, start_str_no_hour, end_str_no_hour):
        stations = AppState.selected_tables
        max_ng = -1
        max_ng_station = None
        for station in stations:
        ...
        if max_ng_station is None or max_ng == 0:
            ax.text(0.5, 0.5, "No NG data available", ha='center', va='center', fontsize=10)
            ax.set_title("NG Count by Model", fontsize=10, fontweight='medium')
            ax.set_facecolor('#f8fafc')
            return

        df = AppState.retrieved_dfs.get(max_ng_station)
        if df is None or 'Model' not in df.columns or df.empty:
            ax.text(0.5, 0.5, "No Model column available", ha='center', va='center', fontsize=10)
            ax.set_title("NG Count by Model", fontsize=10, fontweight='medium')
            ax.set_facecolor('#f8fafc')
            return
        ...
        ng = df[df["Result"].str.upper() == "NG"]
        if ng.empty:
            ax.text(0.5, 0.5, "No NG data", ha='center', va='center', fontsize=10)
            ax.set_title("NG Count by Model", fontsize=10, fontweight='medium')
            ax.set_facecolor('#f8fafc')
            return

        model_counts = ng['Model'].value_counts().sort_values(ascending=False)
        bars = ax.bar(np.arange(len(model_counts)), model_counts.values, color='#ef4444')
        ax.set_title(f"NG Count by Model ", fontsize=10, fontweight='medium')
        ax.set_xlabel("Model", fontsize=8)
        ax.set_ylabel("NG Count", fontsize=8)
        ax.set_xticks(np.arange(len(model_counts)))
        ax.set_xticklabels(model_counts.index, rotation=45, ha='right', fontsize=6)
        ax.grid(True, axis='y', linestyle='--', alpha=0.7, color='#e2e8f0')
        ax.tick_params(axis='both', which='major', labelsize=9)
        max_height = model_counts.max() if not model_counts.empty else 0
        for bar in bars:
            height = bar.get_height()
            if height > 0:
                ax.text(bar.get_x() + bar.get_width()/2, height + max_height * 0.02, str(int(height)), 
                        ha='center', va='bottom', color='#1f2a44', fontweight='medium', fontsize=8)
        ax.set_ylim(0, max_height * 1.1)

    def _plot_root_cause_for_station(self, ax, station, model):
        df = AppState.retrieved_dfs.get(station)
        ...
        try:
            ...
            ax.text(0.5, 0.5, "No features defined", ha='center', va='center', fontsize=8)
            ax.set_title(f"Top 5 Root Causes for {station}", fontsize=8, fontweight='medium')
            ax.set_facecolor('#f8fafc')
            return
        fail_counts = pd.Series(
            {
                col: (ng[col].astype(str).str.lower() == "fail").sum()
                for col in features
                if col in ng.columns
            }
        )
        if fail_counts.empty:
            ax.text(0.5, 0.5, "No fail counts", ha='center', va='center', fontsize=8)
            ax.set_title(f"Top 5 Root Causes for {station}", fontsize=8, fontweight='medium')
            ax.set_facecolor('#f8fafc')
            return
        fail_counts = fail_counts.sort_values(ascending=False).head(5)
        bars = ax.bar(np.arange(len(fail_counts)), fail_counts.values, color='#ef4444')
        ax.set_title(f"Top 5 Root Causes for {station}", fontsize=8, fontweight='medium')
        ax.set_xlabel("Root Cause", fontsize=6)
        ax.set_ylabel("Count", fontsize=6)
        ax.set_xticks(np.arange(len(fail_counts)))
        ax.set_xticklabels(fail_counts.index, rotation=45, ha='right', fontsize=5)
        ax.grid(True, axis='y', linestyle='--', alpha=0.7, color='#e2e8f0')
        ax.tick_params(axis='both', which='major', labelsize=8)
        ax.set_facecolor('#f8fafc')
        max_height = fail_counts.max()
        for bar in bars:
            height = bar.get_height()
            if height > 0:
                ax.text(bar.get_x() + bar.get_width()/2, height + max_height * 0.02, str(int(height)), 
    ...
    def update_for_new_data(self):
        ...
        for row, station in enumerate(stations):
            ...
            if station in AppState.features:
                models = list(AppState.features[station].get("models", {}).keys())
    ...
    def save_html_report(self):
        ...
        if not selected:
            QMessageBox.information(self, "Info", "No stations selected for report.")
            return
        ...
        try:
            ...
            fig = Figure(figsize=(5, 3))
            ax = fig.add_subplot(111)
            self._plot_ng_by_model(ax, start_str_no_hour, end_str_no_hour)
            buf = BytesIO()
            fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
            buf.seek(0)
            img_data = base64.b64encode(buf.read()).decode('utf-8')
            ...
            for station, model in selected:
                ...
                fig = Figure(figsize=(3.5, 2.2))
                ax = fig.add_subplot(111)
                self._plot_root_cause_for_station(ax, station, model)
                buf = BytesIO()
                fig.savefig(buf, format='png', bbox_inches='tight', dpi=100)
                buf.seek(0)
                img_data = base64.b64encode(buf.read()).decode('utf-8')
            ...
            # Save both analyzed and non-analyzed retrieved tables if requested
            if include_data:
                ...
                for station, df in AppState.retrieved_dfs.items():
                    ...
                    try:
                        ...
                        log(f"Saved data for {station} to {file_path}")
                    except Exception as e:
                        log(f"Failed to save data for {station}: {e}", "ERROR")
